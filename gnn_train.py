...
parser.add_argument('--ppi_path', default='./data/9606.protein.actions.all_connected.txt', type=str,
                    help="ppi path")
parser.add_argument('--pseq_path', default='./data/protein.STRING_all_connected.sequences.dictionary.tsv', type=str,
...
parser.add_argument('--train_valid_index_path', default='./train_valid_index_json/.gitkeep', type=str,
                    help='cnn_rnn and gnn unified trait and valid ppi index')
parser.add_argument('--use_lr_scheduler', default=0.001,type=boolean_string,
...
def train(model, graph, ppi_list, loss_fn, optimizer, device,
          result_file_path, summary_writer, save_path, Gadj2,
          batch_size=512, epochs=300,scheduler=None,
          got=False): ####
    global_step = 0
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps):
            ...
            if got:
                output = model(graph.x, graph.edge_index_got, train_edge_id, Gadj2)
                label = graph.edge_attr_got[train_edge_id]
            else:
                output = model(graph.x, graph.edge_index, train_edge_id, Gadj2)
        ...
        with torch.no_grad():
            for step in range(valid_steps):
                ...
                output = model(graph.x, graph.edge_index, valid_edge_id, Gadj2)
...
def main():
    ...
    # graphadj = ppi_data.adj
    graphadj2 = ppi_data.adj2 ####
    # print(graphadj2.shape)
    # exit(0)
    # graphadj3 = ppi_data.adj3 ####
    ...
    # print(graphadj.shape)
    # print(graph.edge_index_got.shape)
    # exit(0)
    ...
    train(model, graph, ppi_list, loss_fn, optimizer, device,
          ...
          got=args.graph_only_train, Gadj2=graphadj2) ####
